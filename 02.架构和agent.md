...
## 7. 工程实现：Node 详解（apps/api/weekend_agent）
...
| **Tool 层** | （无 node） | `tools/registry.py` + `tools/mock_client.py` | 给 Researcher / Executor 调用的 Mock 后台；对接 01 §6 接口 |
...
- Mock 路由清单与 curl 演示见 `docs/mock-api.md`；主服务亦挂载 `/mock-meituan/*`（`weekend_agent/app.py`）。
