...
```bash
cd apps/api
python -m weekend_agent.demo --scene family
# 顶部会显示：Mock 美团：ASGI 内联（默认）
```

### 2. 主进程附挂

直接 `uvicorn weekend_agent.app:app`，mock 路由会被挂在 `/mock-meituan/*`：

```bash
cd apps/api
python -m uvicorn weekend_agent.app:app --reload
# 评委可 curl 验证：
...
# 终端 A：拉假美团（:8001）
cd apps/api
python -m uvicorn weekend_agent.mock_meituan.app:mock_app --port 8001
...
```bash
python -m weekend_agent.demo --scene family --fail 吃
```
