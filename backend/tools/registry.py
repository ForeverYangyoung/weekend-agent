"""Tool 注册表：节点只认 tool_name，具体执行走 HTTP 打到 Mock 美团。

force_failure 注入：演示开关 `state["force_failure"] = "吃"` 会让
...
from backend.planner.constants import READ_TO_WRITE, STAGE_FAIL, TOOL_PATH
from backend.tools.errors import ToolError
from backend.tools.http_client import post_json

_WRITE_TOOLS = frozenset(READ_TO_WRITE.values())
...
def _resolve_force_fail(tool_name: str, ctx: ToolContext, stage_name: str) -> str | None:
    ...
    if tool_name in _WRITE_TOOLS and ctx.force_failure_stage == stage_name:
        return STAGE_FAIL.get(stage_name)
    return None
...
def _build_payload(tool_name: str, args: dict, ctx: ToolContext, stage_name: str) -> dict:
    ...
    if tool_name == "check_activity_availability":
        return {"poi_id": args["poi_id"], "start": args.get("start", ""), "force_fail": ff}
    if tool_name == "check_table_availability":
        return {
            "poi_id": args["poi_id"], "time": args.get("time", ""),
            "people": int(args.get("people", args.get("ppl", 4))), "force_fail": ff,
        }
    ...
    if tool_name == "buy_ticket":
        return {
            ...
            "idempotency_key": key, "force_fail": ff,
        }
    ...
    if tool_name == "order_addon":
        return {
            "poi_id": args["poi_id"], "idempotency_key": key,
            "items": args.get("items"), "force_fail": ff,
        }
