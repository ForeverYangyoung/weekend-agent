...
- 第一轮回复前，优先读取 `.super-dev/WORKFLOW.md` 与 `output/weekend-agent-bootstrap.md`（若存在），把其中的初始化契约视为当前仓库的显式 bootstrap 规则。
...
2. 再生成三份核心文档，再进入编码阶段：
   - `output/*-prd.md`（摘要须对齐 `01-题目工程拆解.md`）
   - `output/*-architecture.md`（摘要须对齐 `02.架构和agent.md`）
   - `output/*-uiux.md`
